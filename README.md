...
To build the module, first clone the repository and run make:

	git clone https://github.com/max1220/lua-db
	cd lua-db
	make

You can also build for lua5.3:
...
You need to put the cloned folder somewhere in lua's package.path and
package.cpath. To do that, you could:

	sudo mkdir -p /usr/local/share/lua/5.1/
	sudo mkdir -p /usr/local/lib/lua/5.1/
	sudo ln -s /path/to/lua-db /usr/local/share/lua/5.1/
	sudo ln -s /path/to/lua-db /usr/local/lib/lua/5.1/
...
Close the drawbuffer, freeing memory
	...
	db:load_data_rgba_nonzeroalpha(data)

Loads the rgba data, but don't set pixels if the data's alpha value is zero



	db:load_data_bgra(data)

Loads bgra data. Otherise same as load_data_rgba.



	db:load_data_rgb(data)

Loads rgb data. The required data size does not match db:bytes_len.



	db:fill_triangle(x1,y1,x2,y2,x3,y3,r,g,b,a)

Fills the specified triangle with the color.
...
The color codes can be generated by the term.lua module.
	...
	Braile.draw_db(db, threshold, color, bpp24)
	Braile.draw_db_precise(db, threshold, bpp24)
...
colored as described above. draw_db_precise uses a different algorhithm
for setting the color values for each character(and does not
set the background)
